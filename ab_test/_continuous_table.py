"""Shared base class for the frequentist and Bayesian normal tables.

Holds the storage, export (DataFrame/list/numpy), (de)serialization, string
rendering, and forest-plot machinery common to both
:class:`~ab_test.frequentist_normal.normal_table.NormalTable` and
:class:`~ab_test.bayesian_normal.normal_table.BayesianNormalTable`.
...
class BaseContinuousTable:
    """Common storage, export, and plotting for continuous tables."""
    ...
    def deserialize(self, serial: dict[str, Any]) -> Self:
        ...
        self.means = [v["means"] for v in serial["table"].values()]
        self.variances = [v["variances"] for v in serial["table"].values()]
